...
\frac{|u|}{(1+W)}$. In addition **the supremum is attained if and only if $u = \frac{dP}{dR}$**. Hence if $(\hat\phi, \hat\psi)$ is a solution to the dual formulation, $\hat\pi$ is the solution to the static formulation and $\hat P$ is the solution to the path formulation, we must have (modulo some technicality) \begin{align}
...
\end{align} where $L$ is the generator of the reversible process $R$ (which does not differentiate between the forward and backward case) and $\Gamma(u, v):= L(uv) - uLv - vLu$ is the **carré du champ operator** of $R$ which measures the tendency of $L$ from being a derivation.
:::
