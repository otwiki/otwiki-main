...
Note that along any smooth curve $x_t$ it holds that
...
\par Now if $F$ is $\lambda$-convex, the inequality that characterizes the gradient is
...
\par The first notion is the generalization of "derivative" in metric space, so-called **metric derivative** defined as follows: Given a curve $x:[0, T] \rightarrow X$ valued in a metric space, $$
...
\par The second concept is the generalization of convexity. A function $F$ is called **geodeiscally convex** if it is convex along any geodesic: $$F(x(t)) \leq(1-t) F(x(0))+t F(x(1)).$$ Similarly we define **geodeisc $\lambda$-convex** if 
$$
F(x(t)) \leq(1-t) F(x(0))+t F(x(1))-\lambda \frac{t(1-t)}{2} d^2(x(0), x(1)) .
$$
\par Lastly we define the metric version of $|\nabla F|$ as **slope**:
Let $F: X \rightarrow \mathbb{R} \cup\{+\infty\}$ and $x \in X$ be such that $F(x)<\infty$. Then the slope $|\nabla F|(x)$ of $F$ at $x$ is:
...
\par (Energy Dissipation Equality definition of GF - EDE) Let $E: X \rightarrow \mathbb{R} \cup\{+\infty\}$ and let $\bar{x} \in X$ be such that $E(\bar{x})<\infty$. We say that $[0, \infty) \ni t \mapsto x_t \in X$ is a Gradient Flow in the EDE sense starting at $\bar{x}$ provided it is a locally absolutely continuous curve, $x_0=\bar{x}$ and
