...
The [Kantorovich dual problem](https://otwiki.github.io/otwiki-main/KDualProblem.html) is a common reformulation of the [Kantorovich problem](https://otwiki.github.io/otwiki-main/Kantorovich_Problem.html). The case $c(x,y) =\frac{1}{2} |x-y|^2$ is an important special case for several reasons. First off, the minimum cost is the [2-Wasserstein distance](1Wasserstein.qmd). From the point of view of mathematical analysis, the quadratic cost is special because the $c$-transform for $c(x,y)= \frac{1}{2} |x-y|^2$ is closely related to convex conjugates, and many tools from convex analysis can be used. Moreover, in the quadratic case the Monge optimal transport map is of the form $T = \nabla \phi$ for a convex function $\phi$ (as discussed in the section on [Brenier's theorem](Monge.qmd)).
...
This is Theorem 1.40 in [@santambrogio2015optimal] (p.32), a proof can be found there. For more general $c(x,y)$, the dual problem still has a solution (and the maximum of the dual problem is equal to the infimum in the primal problem) as is discussed [here](KDualProblem.qmd). However, much of the proof in the non-quadratic case relies on theory of the $c$-transform instead of convex analysis. The book [@santambrogio2015optimal] is also a good resource to see the proof in the general case.
