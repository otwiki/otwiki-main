...
Let us introduce the mathematical framework and notation for a neural network with a single hidden layer.[^ref-figalli] Let $D \subset \mathbb{R}^d$ be open . The set $D$ represents the space of inputs into the network. There is some unknown function $f:D \rightarrow \mathbb{R}$ which we would like to approximate. Let $N  \in \mathbb{N}$ be the number of neurons in the hidden layer. Define
...
where $\nabla g$ is the gradient of g.[^ref-schiebinger]
...
where $D_v g$ is the directional derivative of $g$ in the direction $v$. Motivated by this and using the [Riemannian structure of the space of probability measures](RiemannianStructureOriginal.qmd), we can define the a notion of gradient for our loss functional $F$.

Note that one can define [gradient flows in a general Hilbert space](GFHilbert.qmd).
...
where $\frac{\delta F}{\delta \mu_*}$ is the first variation of $F$ at $\mu_*$ [^ref-ambrosio] (also called the [functional derivative](https://en.wikipedia.org/wiki/Functional_derivative#cite_note-ParrYangP246A.2-3)).
...
where $v_*$ is the unique velocity vector corresponding to $\mu_*$ by the equivalence of absolutely continuous curves and solutions of [the continuity equation](CtyEqn.qmd). Now by the definition of the inner product structure of $(\mathcal{P}_2(\mathbb{R}^d), W_2)$, we have
...
[Generalization error](https://en.wikipedia.org/wiki/Generalization_error) is how well a model generalizes for a new data not in your training data set. In practice, gradient descent does generalize really well, but it is an open problem to provide a theoretical framework to guarantee generalization. Wang, Meng, Chen and Liu 2021 made progress on studying the implicit regularization of the algorithm, and provide a framework for convergence. [^ref-wang]

## References

[^ref-figalli]: [X. Fernandez-Real and A. Figalli, *The Continuous Formulation of Shallow Neural Networks as Wasserstein-Type Gradient Flows*](https://people.math.ethz.ch/~afigalli/lecture-notes-pdf/The-continuous-formulation-of-shallow-neural-networks-as-wasserstein-type-gradient-flows.pdf)

[^ref-schiebinger]: [G. Schiebinger, *Gradient Flow in Wasserstein Space*](https://personal.math.ubc.ca/~geoff/courses/W2019T1/Lecture16.pdf)

[^ref-ambrosio]: [L. Ambrosio, N. Gigli, G. Savaré, *Gradient Flows in Metric Spaces and in the Space of Probability Measures*](http://www2.stat.duke.edu/~sayan/ambrosio.pdf)

[^ref-wang]: [B. Wang, Q. Meng, W. Chen, T. Liu, *The Implicit Regularization for Adaptive Optimization Algorithms on Homogeneous Neural Networks*](https://arxiv.org/pdf/2012.06244.pdf)
