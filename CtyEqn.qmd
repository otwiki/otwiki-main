...
First, because of the intuition, we will introduce the definition of the continuity equation in fluid mechanics. The exposition in this section will follow the book by Chorin and Marsden[^ref-marsden].
...
Here, we will present definitions and reasoning from book by F.Santambrogio[^ref-santambrogio].
...
The main goal of the classical optimal transport theory is how to find the least expensive way to move one measure to the another one. For more information, look at [Monge Problem](Monge.qmd).So, we have to impose initial and terminal conditions on measures, for example $\mu_{0} = \mu$, and $\mu_{1} = \nu.$ Then, our equation becomes $\int_{0}^{T}\int_{\Omega} [\partial_{t}\phi + \nabla\phi\cdot v_{t}] d\mu_{t} dt = \int_{\Omega}\phi(T,x)d\nu(x) - \int_{\Omega}\phi(0,x)d\mu(x),$ for all $\phi \in C_{c}^{1}([0,T]\times \overline{\Omega}).$
...
Some connections between these two types of solutions are given in the following propositions.
   ...
   **Proposition 1.**, (p.124,[^ref-santambrogio]) Distributional and weak solutions are equivalent. Every weak solution is a distributional solution. On the other hand, every distributional solution admits a representative (a.e. equal), that is weakly continuous and a weak solution.

   **Proposition 2.**,(p.124,[^ref-santambrogio]) Let $\mu$ be the Lipschitz function in $(t,x)$ and $v$ be the Lipschitz function in $x.$ Suppose that the continuity equation is satisfied in the weak sense. Then it is satisfied in a.e. sense.
...
The following theorem can be found at the book by L.Ambrosio, E.Brué, and D.Semola[^ref-ambrosio].

   **Theorem (Benamou-Brenier Formula).**[^ref-ambrosio] Let $\mu, \nu \in \mathcal{P}_{2}(\mathbb{R}^{d})$. Then $W_{2}^{2}(\mu, \nu)=\min_{(\mu(t),\nu(t))} \{\int_{0}^{1} |v(\cdot,t)|_{L^{2}(\mu(t))}^{2}dt \quad | \quad \partial_{t}\mu+\nabla\cdot(v\mu)=0,\quad \mu(0)=\mu,\quad \mu(1)=\nu \}.$

This formula is important for defining the Riemannian structure of our Wasserstein space. In particular, this is related to the fact that the distance minimizing curves on Riemannian manifolds are geodesics. The property at play here is the fact that geodesics minimize the action of the Riemannian metric. In other words, an optimal transport plan between measures $\mu$ and $\nu$ is actually related to a geodesic connecting these measures on a Riemannian manifold. The Benamou-Brenier Formula implies that the correct action on this space is $\min_{(\mu(t),\nu(t))} \int_{0}^{1} |v(\cdot,t)|_{L^{2}(\mu(t))}^{2}dt$ where $v(\cdot, t)$ gives the velocity of the curve $\mu$. This implies the correct Riemannian metric for our Wasserstein space. You can see more at [Formal Riemannian Structure of the Wasserstein metric](RiemannianStructureOriginal.qmd).

In addition, using the continuity equation we can describe geodesics in the Wasserstein space. For more details look at [Geodesics and generalized geodesics](GeoGenGeo.qmd).

# References

[^ref-marsden]: [A.J.Chorin, J.E.Marsden, *A Mathematical Introduction to Fluid Mechanics*, Chapter 1, pages 1-11](https://link.springer.com/book/10.1007/978-1-4612-0883-9)

[^ref-ambrosio]: [L.Ambrosio, E.Brué, D.Semola, *Lectures on Optimal Transport*, Lecture 16.1., pages 183-189](https://link.springer.com/book/10.1007/978-3-030-72162-6)

[^ref-santambrogio]: [F.Santambrogio, *Optimal Transport for Applied Mathematicians*, Chapter 4, pages 123-126](https://link.springer.com/book/10.1007/978-3-319-20828-2)
