...
As the Kantorovich problem is a linear program, we may consider its dual. In the discrete case, the solution to the dual problem equals the solution to the primal problem, as discussed in the article [Kantorovich dual problem](KDualProblem.qmd). Our dual problem reduces to $$ \max \limits_{(f, g) \in R(c)} \left < f, a \right > + \left < g, b \right > , $$ where $R(c) := \left \{ (f, g) \in \mathbb{R}^n \times \mathbb{R}^m :  \text{ for all } 1 \leq i \leq n, 1 \leq j \leq m, f_i + g_j \leq c_{ij} \right \}.$ Again, we know that this maximum is achieved; see the article above for the rationale.
