...
On Euclidean space, it is known that the solution to the heat equation can be viewed as a gradient flow. In other words, consider the heat equation $$ \begin{cases} 
          ...
          u(0,\cdot)=u_0(\cdot) . \\  
       \end{cases} 
    $$ We define the energy functional $\mathcal{F}$ by $$ \mathcal{F}(f)=\int_{\mathbb{R}^d}f(x)\ln(f(x))dx . $$ We restrict the domain of $\mathcal{F}$ to a suitable class of functions on $\mathbb{R}^d$ such that $\mathcal{F}(f)$ is well-defined and finite. Consider the metric space $(P_2(\mathbb{R}^d), W_2)$, where $W_2$ denotes the 2-Wasserstein metric, and $$ P_2(\mathbb{R}^d) = \left \{ \mu \in P(\mathbb{R}^d) : \int_{\mathbb{R}^d} |x|^2 d \mu(x) < \infty \right \} . $$ This metric space has a formal Riemannian structure (see [this article](RegularityOTMap.qmd) for more information), so the operator $\nabla_{W_2}$ is well-defined. Based on the seminal work of Jordan, Kinderlehrer and Otto in [@JKO], we know that the solution to the heat equation, $u(t, x)$, satisfies $$
...
\end{cases} $$ Readers who are familiar with the [Benamou-Brenier formula](CtyEqn.qmd) will note the similarity between this definition and that formula. We have the following:
