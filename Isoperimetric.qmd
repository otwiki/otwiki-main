...
For this part I will follow the first chapter in [^ref-restored-10]. In some sense, it can be seen as a multidimension generalization of monotone rearrangement. Take any two measures $\mu,\nu \in \mathcal{P}(\mathbb{R})$ and define
...
. We can iterate the same construction and obtain a Knothe map on $\mathbb{R}^n$, the recursive nature of this definition is well described in [^ref-restored-11], the formal construction can be found here [^ref-restored-12].
...
## The isoperimetric inequality in $CD(0,N)$ spaces

There is a more general version of the classic isoperimetric inequality for Riemannian manifolds and even more in general for $CD(0,N)$spaces. In the Riemannian setting this result is proved first by Simon Brendle in Corollary 1.3 of[^ref-restored-13], the proof is based on PDE techniques, notably on the so called ABP-method (Aleksandrov-Bakelman-Pucci estimates, a standard reference for this would be [^ref-restored-14]).

Since OMT-theory has been worked very well in a much more general setting of metric measure spaces, we can construct the so called $CD(K,N)$ spaces where $K$ is a lower bound for the Ricci curvature and $N$ is an upper bound for the dimension (Wiki article on Ricci Curvature and OMT [RicciCurvature](RicciCurvature.qmd)). This has been worked out independently by Lott and Villani: [^ref-note-1] and Sturm in two very beautiful papers: [^ref-restored-15] and [^ref-restored-16]. The idea is to express the curvature in terms of the convexity of a specific entropy function defined on the space. It is also well known that even in the case of non negative curvature, i.e. $CD(0,N)$ without an additional condition, a general sharp isoperimetric inequality cannot hold! This can be found in two papers from Milman: [^ref-restored-17] and [^ref-restored-18]. To my knowledge, one of the most general result in this direction is Theorem 1.1 in [^ref-restored-19]. In this case the condition is the so called Euclidean volume growth condition at infinity. If we denote with $B_x(r)=\{y \in M: d(x,y)<r \}$, by an generalization of the classical Bishop-Gromov volume growth inequality, theorem 2.3 of[^ref-restored-20], it follows that the map $r \mapsto \frac{m(B_x(r))}{r^N}$ is non increasing on $(0,\infty)$ for any $x \in M$. The Euclidean volume growth at infinity is a the positivity of the following limit:
...
(see [^ref-restored-22] for a rigorous introduction to functions of bounded variations and perimeters of measurable sets. The main connection between these two perimeter can be seen as follows: Let $K=B_1$ the unit ball in the Euclidean space around the origin. Then, for any $x \in \partial E$, we have that

$\|\nu_E(x)\|_*=\sup\{ \langle \nu_E(x), v\rangle:\quad  v\in B_1\}=1$, where the last equality follows from the Cauchy Schwarz inequality, being

$|\langle v,w\rangle| \leq \|v\| \|w\|$ where equality occurs if and only if $v=\lambda w$ for some $\lambda \in \mathbb R$. Then we have that $P_K(E)=\int_{\partial E} d \mathcal H^{N-1}=P(E).$ Notice, though, that the anisotropic perimeter does not obey as nice properties as the classical perimeter:

#### Easy Example

 Let us now consider an easy example for the anisotropic perimeter. Let us consider the case $K=\{(x,y)\in \mathbb R^2:\quad |x|^2+|y-\varepsilon|^2< \varepsilon ^2\}\cup [-\delta ,\delta ]\times  [-5,\varepsilon]\cup \{(x,y)\in \mathbb R^2:\quad |x|^2+|y+5|^2< 1\},$
...
The proof of that inequality really follows with similar arguments as in the classical case. We restrict ourselves to the case where the Transport map is smooth as a gradient of a strictly convex function. For a general proof see [^ref-note-4] We follow the proof given in [^ref-note-5] Indeed, as in the proof of the classical isoperimetric inequality, we get a transport map $T$ which now pushes forward the density $\frac{\chi_E}{|E|}dx$ to the density $\frac{\chi_K}{|K|}dx$, where $dx$ denotes the $N$ dimensional Lebesgue measure on $\mathbb R^N$ . By our assumption on the transport map, we know that there are positive $\lambda_i:\mathbb R^N \rightarrow \mathbb R$ for $i=1\dots,n$ such that
...
as desired. To see a proof of the equality case, we refer the reader to \[[^ref-note-6], Chapter 1\].

[^ref-note-1]: J. Lott, C. Villani, Ricci curvature for metric measure spaces via optimal transport. Ann. of Math. (2) 169 (3) (2009)

[^ref-note-2]: Figalli, A., Maggi, F. & Pratelli, A. A mass transportation approach to quantitative isoperimetric inequalities. Invent. math. 182, 167–211 (2010). https://doi.org/10.1007/s00222-010-0261-z

[^ref-note-3]: Figalli, A., Maggi, F. & Pratelli, A. A mass transportation approach to quantitative isoperimetric inequalities. Invent. math. 182, 167–211 (2010). https://doi.org/10.1007/s00222-010-0261-z

[^ref-note-4]: Figalli, A., Maggi, F. & Pratelli, A. A mass transportation approach to quantitative isoperimetric inequalities. Invent. math. 182, 167–211 (2010). https://doi.org/10.1007/s00222-010-0261-z

[^ref-note-5]: A. Figalli, Quantitative isoperimetric inequalities, with applications to the stability of liquid drops and crystals, Concentration, functional inequalities and isoperimetry, 77-87, Contemp. Math., 545, Amer. Math. Soc., Providence, RI, 2011.

[^ref-note-6]: A. Figalli, Quantitative isoperimetric inequalities, with applications to the stability of liquid drops and crystals, Concentration, functional inequalities and isoperimetry, 77-87, Contemp. Math., 545, Amer. Math. Soc., Providence, RI, 2011.

[^ref-restored-7]: V.D. Milman, G. Schechtman, Asymptotic Theory of Finite-Dimensional Normed Spaces, with an appendix by M. Gromov, Lecture notes in Mathematics, vol. 1200 (Springer, Berlin, 1986)

[^ref-restored-8]: Herbert Knothe. "Contributions to the theory of convex bodies.." Michigan Math. J. 4 (1) 39 - 52, 1957

[^ref-restored-9]: F. Santambrogio. Optimal Transport for Applied Mathematicians. Calculus of Variations, PDEs and Modeling (2015)

[^ref-restored-10]: A. Figalli, F. Glaudo An Invitation to Optimal Transport, Wasserstein Distances, and Gradient Flows

[^ref-restored-11]: Luigi Ambrosio, Elia Brué, Daniele Semola - Lectures on Optimal Transport Remark 5.10 pag. 52

[^ref-restored-12]: F. Santambrogio. Optimal Transport for Applied Mathematicians. Calculus of Variations, PDEs and Modeling (2015) pag. 67-72

[^ref-restored-13]: S. Brendle, Sobolev inequalities in manifolds with nonnegative curvature (2020) pag. 2

[^ref-restored-14]: X. Cabr´e, X. Ros-Oton, J. Serra, Sharp isoperimetric inequalities via the ABP method. J. Eur. Math. Soc. 18 (2016)

[^ref-restored-15]: K.-T. Sturm, On the geometry of metric measure spaces. I, Acta Math. 196 (1) (2006), 65–131.

[^ref-restored-16]: K.-T. Sturm, On the geometry of metric measure spaces. II, Acta Math. 196 (1) (2006), 133–177.

[^ref-restored-17]: E. Milman, Sharp isoperimetric inequalities and model spaces for the curvature-dimension-diameter condition. J. Eur. Math. Soc. 17 (5) (2015)

[^ref-restored-18]: E. Milman, Beyond traditional curvature-dimension I: new model spaces for isoperimetric and concentration inequalities in negative dimension. Trans. Amer. Math. Soc. 369 (2017), no. 5, 3605–3637

[^ref-restored-19]: Z. M. Balogh, A.Kristály Sharp geometric inequalities in spaces with nonnegative Ricci curvature and Euclidean volume growth (2021)

[^ref-restored-20]: E. Milman, Beyond traditional curvature-dimension I: new model spaces for isoperimetric and concentration inequalities in negative dimension. Trans. Amer. Math. Soc. 369 (2017), no. 5, 3605–3637

[^ref-restored-21]: C. Herring, Some theorems on the free energies of crystal surfaces, Phys. Rev. 82 (1951), 87-93.

[^ref-restored-22]: L. Ambrosio, N. Fusco, D. Pallara: Functions of Bounded Variation and Free Discontinuity Problems, Oxford University Press, 2000.
