...
The **Moreau-Yosida regularization** is a technique used to approximate lower semicontinuous functions by [Lipschitz functions](https://en.wikipedia.org/wiki/Lipschitz_continuity). An important application of this result is to prove Portmanteau's Theorem, which states that integration against a lower semicontinuous and bounded below function is lower semicontinuous with respect to the [narrow convergence](ConvergenceMetrizability.qmd) in the space of [probability measures](https://en.wikipedia.org/wiki/Probability_measure).
...
A function $g : X \to (-\infty,+\infty]$ is said to be **proper** [^ref-ot] if it is not identically equal to $+\infty$, that is, if there exists $x \in X$ such that $g(x) < +\infty$. The **domain** $D(g)$ of $g$ is the set

$D(g) := \{ x \in X | g(x) < +\infty \}$.

For a given function $g : X \to (-\infty,+\infty]$ and $k \geq 0$, its **Moreau-Yosida regularization** [^ref-ot] $g_k : X \to [-\infty,+\infty]$ is given by

$g_k(x) := \inf\limits_{y \in X} \left[ g(y) + k d(x,y) \right].$

The distance term $d(x,y)$ may often be raised to a positive exponent $p$, in particular $p = 2$. For example, when $X$ is a [Hilbert space](https://en.wikipedia.org/wiki/Hilbert_space) [^ref-bc] [^ref-ags], $g_k$ is taken to be
...
**Theorem (Portmanteau).** [^ref-ot] [^ref-s] Let $(X,d)$ be a Polish space, and let $g : X \to (-\infty,+\infty]$ be lower semicontinuous and bounded below. Then the functional $\mu \mapsto \int_X g \, \mathrm{d}\mu$ is lower semicontinuous with respect to [narrow convergence](ConvergenceMetrizability.qmd) in $\mathcal{P}(X)$, that is
...
Taking $k \to \infty$, [Fatou's Lemma](https://en.wikipedia.org/wiki/Fatou%27s_lemma) ensures that
...
The curious epithet attached to the above theorem is due to [Billingsley](https://en.wikipedia.org/wiki/Patrick_Billingsley) [^ref-billingsley], with a citation to a Jean-Pierre Portmanteau's *Espoir pour l'ensemble vide?* published in *Annales de l'Université de Felletin* in 1915. This is believed to be a fictional citation made as a play on words [^ref-pages].

-  The publication date is far too early; [Kolmogorov's probability axioms](https://en.wikipedia.org/wiki/Probability_axioms) were published in 1933. [^ref-kolmogorov]

-  [Felletin](https://en.wikipedia.org/wiki/Felletin) is a small town in central France with no university, and there is no record of a Jean-Pierre Portmanteau aside from this citation.
...
The Moreau-Yosida regularization is a specific case of a type of convolution, and many of the above results follow from this generalization. This material is adapted from Bauschke-Combettes Chapter 12 [^ref-bc], where the setting is over a Hilbert space instead of a more general Polish space.
...
Then the following hold for all $k \in (0,+\infty)$ and $x \in \mathcal{H}$:

-  $D(g_k) = \mathcal{H}$,

-  for $0 < k_1 \leq k_2 < +\infty$, $\inf\limits_{y \in \mathcal{H}} g(y) \leq g_{k_1}(x) \leq g_{k_2}(x) \leq g(x)$,

-  $\inf\limits_{x \in \mathcal{H}} g_k(x) = \inf\limits_{x \in \mathcal{H}} g(x)$,

-  $g_k(x) \searrow \inf\limits_{y \in \mathcal{H}} g(y)$ as $k \downarrow 0$, and

-  $g_k$ is bounded above on every ball in $\mathcal{H}$.
...
[^ref-ot]: Craig, Katy C. Lower Semicontinuity in the Narrow Topology. Math 260J. Univ. of Ca. at Santa Barbara. Winter 2022.

[^ref-bc]: Bauschke, Heinz H. and Patrick L. Combettes. *Convex Analysis and Monotone Operator Theory in Hilbert Spaces, 2nd Ed.* Ch. 12. Springer, 2017.

[^ref-ags]: Ambrosio, Luigi, Nicola Gigli, and Giuseppe Savaré. *Gradient Flows in Metric Spaces and in the Space of Probability Measures.* Ch. 3.1. Birkhäuser, 2005.

[^ref-s]: Santambrogio, Filippo. *Optimal Transport for Applied Mathematicians: Calculus of Variations, PDEs, and Modeling* Ch. 1.1. Birkhäuser, 2015.

[^ref-billingsley]: Billingsley, Patrick. *Convergence of Probability Measures, 2nd Ed.* John Wiley & Sons, Inc. 1999.

[^ref-pages]: Pagès, Gilles. *Numerical Probability: An Introduction with Applications to Finance.* Ch. 4.1. Springer, 2018.

[^ref-kolmogorov]: Kolmogorov, Andrey (1950) 1933. Foundations of the theory of probability. New York, USA: Chelsea Publishing Company.
