...
$$
\max_{(\psi, \varphi) \in R(c)} \Big\{ \int_X \psi d\mu + \int_Y \varphi d\nu \Big\}
$$
...
$$
\max_{\varphi \in \mathbb{R}^m} \Big\{ \mathcal{E}(\varphi) = \int_X \varphi^c d\mu + \sum_j \varphi_j b_j \Big\}.
$$
...
We use the specific cost function $c(x,y) = \frac{1}{2}|x-y|^2$ here. This is a special case and we may generalize to other cost functions if we desire. When we have this special case, the decomposition of our space $X$ is known as a "power diagram."[^ref-merigot]

## The gradient in finding optimal $\varphi$
...
$$
\frac{\partial \mathcal{E} }{\partial \varphi_j} = - \int_{V_{\varphi}(j)} f(x)dx + b_j
$$
...
Since $\nabla \mathcal{E}(\varphi)_j = 0$ when it attains a maximum, we have the relation between the weights and the measure density that we established in the previous section. Note that the maximum is taken and not the minimum because our function $\mathcal{E}(\varphi)$ is a concave function. The mapping $\varphi \rightarrow \sum_j \varphi_j b_j$ is linear, but because we consider an infimum for $\varphi^c$, the overall function $\mathcal{E}(\varphi)$ is concave.[^ref-merigot]

Once we have discovered our optimal $\varphi_j$, the optimal transport map is is one in which any $x \in V_{\varphi_j}$ is mapped to $y_j$. We have the mapping to a constant over each particular region, making the optimal transport map piecewise constant.[^ref-peyre-and-cuturi]

## Algorithm discussion

We may search for a maximum of value of $\mathcal{E}(\varphi)$ by means of certain algorithms, the first being gradient ascent. Whether or not such an algorithm is capable of being implemented effectively is contingent on the ability to find our power diagram in a practical way. Certain computational geometry[^ref-santambrogio] algorithms allow the cells to be found efficiently. A second suitable algorithm is Newton's method. Using Newton's method to find the zeros of $\frac{\partial \mathcal{E} }{\partial \varphi_j}$, one must compute the second derivative of $\mathcal{E}$, as well as justify certain constraints are met, such as bounded eigenvalues of the Hessian.[^ref-santambrogio]

## References

[^ref-peyre-and-cuturi]: [G. Peyré and M. Cuturi, *Computational Optimal Transport*, Chapter 5.](https://arxiv.org/pdf/1803.00567.pdf)

[^ref-santambrogio]: [F. Santambrogio, *Optimal Transport in Applied Mathematics*, Chapter 6.](https://link-springer-com.proxy.library.ucsb.edu:9443/content/pdf/10.1007%2F978-3-319-20828-2.pdf)

[^ref-merigot]: [Mérigot Q., *A Multiscale Approach to Optimal Transport*, Laboratoire Jean Kuntzmann, Université de Grenoble and CNRS.](https://arxiv.org/pdf/1803.00567.pdf)
