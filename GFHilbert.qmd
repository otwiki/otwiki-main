...
First, we recall the notion of the subdifferential, rewritten from Ambrosio et al.'s definition[^ref-ambrosio-brue-semola].
...
A **gradient flow** $x(t):(0,\infty)\rightarrow\text{dom}(f)$ is a locally absolutely continuous function with the property that $x'(t)\in\partial(f(x(t))$ for almost every $t$ (with respect to Lebesgue measure)[^ref-ambrosio-brue-semola]. Note that the $x(t)$ being locally absolutely continuous is necessary for the existence (almost everywhere) of $x'(t)$[^ref-ambrosio-brue-semola]. It will be particularly useful to identify the starting point of a gradient flow $x(t)$, which is given by $x_0:=\lim_{t\rightarrow 0}x(t)$.
...
**Theorem**[^ref-evans] Let $T$ be a densely defined linear operator on a Banach space (note that $T$ need not be bounded, but we assume $T$ is closed). Denote $\rho(T):=\left\lbrace z\in\mathbb{C}:(z-T) \text{ is invertible and bounded}\right\rbrace$ as the resolvent set of $T$. Then $T$ is the infinitesimal generator of a semigroup $S_t$ if and only if, for each $\lambda>0$, we have $\lambda\in\rho(T)$ and $||(\lambda-T)^{-1}||\leq\frac{1}{\lambda}$
...
which has solution $\phi(t):=S_t\phi_0$[^ref-evans] . In the ODE above, $T$ denotes the linear differential operator in the original linear time evolution PDE.
...
In the notation of Ambrosio et al., the analogue of the resolvent used in the proof of the Brézis-Komura Theorem is $(I+\epsilon T)^{-1}$ where $T$ is a non-linear operator on a Hilbert Space $H$[^ref-ambrosio-brue-semola]. Moreover, the existence argument in classical proof the Brézis-Komura Theorem requires a modification to the generalized resolvent of $T$. This modification is the **Yosida Regularization,**[^ref-ambrosio-brue-semola]

$Y(T,\epsilon):=\frac{I-(I+\epsilon T)^{-1}}{\epsilon}$

where $\epsilon$ is some non-negative parameter. The Yosida Regularization, being Lipschitz as an operator on $H$ with Lipschitz constant $\frac{2}{\epsilon}$ [^ref-brezis], allows one to construct the starting point for a solution in the Brézis-Komura Theorem.[^ref-evans] This is analogous to the [Moreau-Yosida Regularization](MoreauYosida.qmd), where lower semicontinuous function is approximated by a Lipschitz function.

Another way to see the Yosida Regularization is via a time-discretization, the approach first used by Jordan, Kinderlehrer and Otto[^ref-jko]. There are two ways of discretizing a gradient flow, namely Euler schemes, explicit one:
...
If we look at the similar energy functional $T(\rho) = |\rho|_{H^{-1}(\Omega)}^{2}$ for probability density function $\rho,$ we get gradient flow $\frac{\partial \rho}{\partial t} = - \nabla \cdot (\rho \nabla \Delta^{-1}\rho),$ that is studied in Ginzburg-Landau dynamics (see [^ref-villani]).

The Brézis-Komura Theorem is also used to assert the existence of the Riemannian Heat Semigroup[^ref-ambrosio-brue-semola]. This forms the starting point for [connecting optimal transport and ricci curvature](RicciCurvature.qmd).

Denote the Boltzmann's $H$ functional $T(\rho) = \int \rho \log(\rho)$ for probability density function $\rho$. If we use the Wasserstein distance $W_{2}$, then the gradient flow satisfies the [Continuity Equation](CtyEqn.qmd) $\frac{\partial \rho}{\partial t} + \nabla \cdot (v\rho) = 0,$ where $v=\frac{-\nabla \rho}{\rho}.$ Hence, this is also the heat equation, $\frac{\partial \rho}{\partial t} = \Delta \rho.$

## References

[^ref-evans]: L Evans, *Partial Differential Equations*, p. 435-443, p. 562-579

[^ref-ambrosio-brue-semola]: [L Ambrosio, E Brué, D Semola, *Lectures on Optimal Transport*, p. 109-124, p.138, p. 230](https://link.springer.com/book/10.1007/978-3-030-72162-6)

[^ref-brezis]: H.Brezis, *Functional Analysis, Sobolev Spaces and Partial Differential Equations*, p. 181-182

[^ref-jko]: Jordan, R., Kinderlehrer, D., and Otto, F., *The variational formulation of the Fokker-Planck equation*, SIAM J.Math Anal. 29, 1 (1998)

[^ref-santambrogio]: F.Santambrogio, *Optimal Transport for Applied Mathematicians*, p.287

[^ref-villani]: C.Villani, *Topics in Optimal Transportation*, p. 260-261
