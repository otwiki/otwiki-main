...
The intuition for $\mu_n$ is this; 
$$
...
The dual of the Kantorovich problem is
$$\sup \int_X f(x) d\mu (x) + \int_Y g(y) d \mu(y)$$
where the supremum is taken over all $f \in L^1(\mu), g \in L^1(\nu)$ such that $f(x) + g(y) \leq c(x,y)$. Here, 
$$L^1(\mu) = \{ f:X \to X \text{ measurable }: \int_X |f|d \mu < \infty\}.$$
Moreover, we have the follwing theorm (this is Theorem 1.3 in [@villani2021topics], you can find a proof there):

:::{#thm-dualEquiv} 

## Equivalence of Primal and Dual
Suppose that $c: X \times X \to [0,\infty)$ is lower semi-continuous. Then,
$$\sup_{f(x)+ g(y) \leq c(x,y)} \int_X f(x) d\mu (x) + \int_X g(y) d \nu(y)= \inf_{\gamma \in \Gamma(\mu,\nu)} \mathbb{K}(\gamma)
$$
...
Assume $\mu, \nu \in \mathcal{P}(\mathbb{R}^d)$ have finite second moments. That is, assume
$$\int_{\mathbb{R}^d} |x|^2 d \mu(x), \quad\int_{\mathbb{R}^d} |y|^2 d \nu(y)< \infty.
$$
Also, assume the cost function is $c(x,y) = |x-y|^2$. The dual problem be further transformed into a problem of optimizing over just one function. 
$$\sup_{f(x)+ g(y) \leq c(x,y)} \int_X f(x) d\mu (x) + \int_Y g(y) d \mu(y) = \inf_{\phi \in L^1(\mu)} S(\phi)$$
where
$$S(\phi) = \int \phi d \mu + \int \phi^* d \nu$$ {#eq-semidual}
Here, $\phi^*$ is the [convex conjugate](https://en.wikipedia.org/wiki/Convex_conjugate) of $\phi$, defined by
$$
